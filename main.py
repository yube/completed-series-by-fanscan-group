...
def create_montage(images, titles, images_per_row=10):
    ...
    title_text = f"Completely scanlated manga by {group_name}"
    ...
    safe_group_name = re.sub(r'[\\/*?:"<>|]', "", group_name)
    montage.save(f"{safe_group_name}.png")
    print(f"Chart saved as {safe_group_name}.png")

def get_group_id(name):
    url = "https://api.mangaupdates.com/v1/groups/search"
    headers = {"Content-Type": "application/json"}
    response = requests.post(url, headers=headers, json={"search": name})
    try:
        data = response.json()
        return data['results'][0]['record']['group_id']
    except Exception as e:
        print("Error retrieving group ID:", e)
        return None

def get_series_ids_by_group(group_id):
    url = f"https://api.mangaupdates.com/v1/groups/{group_id}/series"
    response = requests.get(url)
    try:
        data = response.json()
        return [s['series_id'] for s in data.get("series_titles", [])]
    except Exception as e:
        print("Error retrieving series list:", e)
        return []
...
if not group_id:
    print("No group ID found. Exiting.")
    exit()

ids = get_series_ids_by_group(group_id)
